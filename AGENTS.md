...
- **CSV menu fix**: Removed `overflow:hidden` from `.header-left` which was clipping the dropdown menu (only top 2 regions visible). Removed hardcoded `font-size:11px` inline style so button sizes with responsive CSS. `showCSVMenu()` now closes the filter popover on mobile to prevent overlap.
