...
- **Auto-visit**: Toggle checkbox in Route tab enables GPS proximity detection (100m, 60s dwell). Stays within 100m of a stop for 60s → auto-marks visit (removes from route, refills pool, sets follow-up). Saves preference to localStorage. High-accuracy GPS with 10s cache — client keeps phone plugged in.
- **CSV menu fix**: Removed `overflow:hidden` from `.header-left` which was clipping the dropdown menu (only top 2 regions visible). Removed hardcoded `font-size:11px` inline style so button sizes with responsive CSS. `showCSVMenu()` now closes the filter popover on mobile to prevent overlap. Changed to `position:fixed` with viewport positioning via `getBoundingClientRect()` so it overlays the map legend.
